...
img_files_list = pickle.load(open("files.pkl", "rb"))
...
if uploaded_file is not None:
    if save_file(uploaded_file):
        ...
        with col1:
            ...
            img_path = img_files_list[img_indicess[0][0]].replace("\\", "/")
            st.image(img_path)

        with col2:
            st.header("II")
            # st.image(img_files_list[img_indicess[0][1]])
            img_path = img_files_list[img_indicess[0][1]].replace("\\", "/")
            st.image(img_path)

        with col3:
            st.header("III")
            # st.image(img_files_list[img_indicess[0][2]])
            img_path = img_files_list[img_indicess[0][2]].replace("\\", "/")
            st.image(img_path)

        with col4:
            st.header("IV")
            # st.image(img_files_list[img_indicess[0][3]])
            img_path = img_files_list[img_indicess[0][3]].replace("\\", "/")
            st.image(img_path)

        with col5:
            st.header("V")
            # st.image(img_files_list[img_indicess[0][4]])
            img_path = img_files_list[img_indicess[0][4]].replace("\\", "/")
            st.image(img_path)
